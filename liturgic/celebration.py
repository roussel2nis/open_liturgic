...
class Celebration(models.Model):
    ...
    def onchange_start_stop_datetime(self):
        if not self.start_datetime and not self.stop_datetime:
            return
        
        if self.stop_datetime:
            self.duration = (datetime.strptime(self.stop_datetime,DEFAULT_SERVER_DATETIME_FORMAT) - datetime.strptime(self.start_datetime,DEFAULT_SERVER_DATETIME_FORMAT)).total_seconds() / 3600
        elif self.duration:
            self.stop_datetime = (datetime.strptime(self.start_datetime,DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(hours=self.duration)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        else:
            return
        
        start = datetime.strptime(self.start_datetime, DEFAULT_SERVER_DATETIME_FORMAT)
        
        self.stop_date = (start + timedelta(hours=self.duration)).strftime(DEFAULT_SERVER_DATE_FORMAT)
        self.stop_datetime = (start + timedelta(hours=self.duration)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        self.start_date = start.strftime(DEFAULT_SERVER_DATE_FORMAT)
